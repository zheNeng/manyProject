...
const getReplacer = (value, allowEmpty) => {
  return function(match) {
    ...
    if (value === null || value === undefined) {
      if (!allowEmpty)
        throw new Error(
          `Path variable ${match} not implemented in this context of qn-webpack plugin: ${input}`
        );
      return "";
    } else {
      return `${value}`;
...
class QiniuPlugin {
  constructor(options) {
    ...
    this.options.path =
      this.options.path == null ? "[hash]" : this.options.path;
    if (!isString(this.options.path))
      throw new Error("qn-webpack plugin: path is invalid");
  }
  ...
  apply(compiler) {
    const uploadFiles = (compilation, callback) => {
      ...
      let batch = this.options.batch || 1000;
      ...
      const execStack = function(err) {
        ...
        if (_files.length) {
          return Promise.all(_files.map(performUpload)).then(
            () => execStack(),
            execStack
          );
        } else {
